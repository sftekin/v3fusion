...
def save_checkpoint(save_dir, model_name, data_dict, step_num=None):
    ...
    if step_num is None:
        output_path_df = os.path.join(save_dir, f"{model_name}_output.csv")
        prob_dict_path = os.path.join(save_dir, f"{model_name}_prob.pkl")
    else:
        output_path_df = os.path.join(
            save_dir, f"{model_name}_output_{step_num}.csv")
        prob_dict_path = os.path.join(
            save_dir, f"{model_name}_prob_{step_num}.pkl")

    # save model
    data_df.to_csv(output_path_df)

    # save probs
    with open(prob_dict_path, "wb") as f:
        pkl.dump(prob_dict, f)
...
def run(args):
    ...
    save_dir = os.path.join("results", "inference",
                            args.task_name, args.dataset_type)
    ...
    questions, generated_outputs, ground_truths, all_probs, tokens = [], [], [], [], []
            ...
            for question, answer in zip(example["question"], example["answer"]):
                ...
                if "InternVL2" not in args.model_name:
                    res_dict = construct_open_ended_prompt(
                        question, prompt_formats, processor=processor)
                    inputs = processor(
                    ...
                    output_txt = processor.decode(
                        output["sequences"][0], skip_special_tokens=True)

                    # we need the following for verification
                    topk_probs, topk_tokens = [], []
                    for i in range(len(output["scores"])):
                        probs = F.softmax(output["scores"][i], dim=-1)
                        vals, indices = torch.topk(probs, k=10)
                        topk_probs.append(vals.cpu())
                        topk_tokens.append([processor.decode([tkn])
                                           for tkn in indices[0]])
                    topk_tokens = np.array(topk_tokens)
                    topk_probs = np.concatenate(topk_probs)
                    all_probs.append(topk_probs)
                    tokens.append(topk_tokens)
                    ...
                    mem_before = process.memory_info().rss / (1024 ** 2)  # in MB
                    start = time.perf_counter()
                    output_txt = model.chat(processor, pixel_values, res_dict["prompt"], generation_config)
                    end = time.perf_counter()
                    times.append(end - start)
                    mem_after = process.memory_info().rss / (1024 ** 2)  # in MB
                    memory_usage.append(mem_after - mem_before)
                ...
                if num_samples % 500 == 0 and num_samples > 0:
                    ...
                    save_checkpoint(save_dir, args.model_name,
    ...
    print(f"Mean latency: {times.mean()*1000:.2f} ms, Mean mem: {memory_usage.mean():.2f}")
    print(f"P50 latency:  {np.percentile(times, 50)*1000:.2f} ms, P50 latency: {np.percentile(memory_usage, 50):.2f}")
    print(f"P90 latency:  {np.percentile(times, 90)*1000:.2f} ms, P90 latency: {np.percentile(memory_usage, 90):.2f}")
    print(f"P99 latency:  {np.percentile(times, 99)*1000:.2f} ms, P99 latency: {np.percentile(memory_usage, 99):.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='inference scripts for the trained models')
    ...
    parser.add_argument("--dataset_type", type=str,
                        default="test", choices=["test", "validation", "train"])
